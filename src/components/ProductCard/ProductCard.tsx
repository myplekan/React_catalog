...
import { actions } from "../../features/favourites";
...
export const ProductCard: React.FC<Props> = ({ product, firstPhone = 0 }) => {
  ...
  const handleSetFavourites = (newValue: Product) => {
    ...
    if (index !== -1) {
      dispatch(actions.remove(newValue))
    } else {
      dispatch(actions.add(newValue))
    }
  };

  const findFavouriteCard = () => {
    return favourites.find((favourite: Product) => favourite.id === id);
  };

  const existingCartItem = cart.find((item: Cart) => item.product.id === product.id);

  const pageWidth = window.innerWidth;

  const cardWidth = pageWidth > 639 && pageWidth < 1200 ? 320 : 288;
  return (
    <div
      className="product-card"
      style={{
        transform: `translateX(${-(cardWidth * firstPhone)}px)`,
        transition: "transform 1000ms",
      }}
    >
      <Link to={`/phones/${phoneId}`} className="product-card__image-link">
        <img
      ...
      <Link to={`/phones/${phoneId}`} className="product-card__title">
        {name}
      </Link>
      ...
      <div className="product-card__button-action">
        {!existingCartItem ? (
          <button
            ...
            type="button"
            onClick={() => handleSetCart(product, cart, dispatch)}
          >
            Add to cart
          </button>
        ) : (
